...
const AccordionItem = React.forwardRef(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-white border-opacity-5", className)}
    {...props}
  />
));
  ...
  ({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "flex flex-1 items-center justify-between py-4     @laptop:py-6 text-sm text-left @laptop:text-xl font-medium transition-all hover:text-blue-primary [&[data-state=open]>svg]:rotate-180",
          className
        )}
        {...props}>
